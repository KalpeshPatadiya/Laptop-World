<html>
<head>

</head>
<body>
<!-- Home Page - Footer Page  -->
<footer class="text-center text-lg-start bg-light text-muted">
        <!-- Section: Links  -->
        <section class="d-flex justify-content-center justify-content-lg-between p-4">
            <div class="container text-center text-md-start mt-2">
                <!-- Grid row -->
                <div class="row mt-3">
                    <!-- Grid column -->
                    <div class="col-md-3 col-lg-4 col-xl-3 mx-auto mb-4">
                        <!-- Content -->
                        <h6 class="text-uppercase fw-bold mb-4">
                            <i class="fas fa-gem me-3"></i>Laptop World
                        </h6>
                        <p>
                            Best website for laptops and their accessories    
                        </p>
                    </div>
                    <!-- Grid column -->

                    <!-- Grid column -->
                    <div class="col-md-2 col-lg-2 col-xl-2 mx-auto mb-4">
                        <!-- Links -->
                        <h6 class="text-uppercase fw-bold mb-4">
                            Companies
                        </h6>
                        <p>
                            <a href="https://www.lenovo.com/in/en/" target="_blank" class="text-reset">Lenovo</a>
                        </p>
                        <p>
                            <a href="https://www.apple.com/in/" target="_blank" class="text-reset">Apple</a>
                        </p>
                        <p>
                            <a href="https://www.dell.com/en-in" target="_blank" class="text-reset">Dell</a>
                        </p>
                        <p>
                            <a href="https://www.asus.com/in/" target="_blank" class="text-reset">Asus</a>
                        </p>
                    </div>
                    <!-- Grid column -->

                    <!-- Grid column -->
                    <div class="col-md-3 col-lg-2 col-xl-2 mx-auto mb-4">
                        <!-- Links -->
                        <h6 class="text-uppercase fw-bold mb-4">
                            Useful links
                        </h6>
                        <p>
                            <a href="http://www.rcti.cteguj.in" target="_blank" class="text-reset">RCTI</a>
                        </p>
                        <p>
                            <a href="https://www.php.net/" target="_blank" class="text-reset">PHP</a>
                        </p>
                        <p>
                            <a href="https://laravel.com/" target="_blank" class="text-reset">Laravel</a>
                        </p>
                        <p>
                            <a href="https://getbootstrap.com/" target="_blank" class="text-reset">Bootstrap</a>
                        </p>
                    </div>
                    <!-- Grid column -->

                    <!-- Grid column -->
                    <div class="col-md-4 col-lg-3 col-xl-3 mx-auto mb-md-0 mb-4">
                        <!-- Links -->
                        <h6 class="text-uppercase fw-bold mb-4">
                            Contact
                        </h6>
                        <p><i class="fas fa-home me-3"></i>Opp Civil, Near Gujarat High Court, S.G.Highway, Sola, Ahmedabad, Gujarat 380060</p>
                        <p>
                            <i class="fas fa-envelope me-3"></i>
                            rcti-abad-dte@gujarat.gov.in
                        </p>
                        <p><i class="fas fa-phone me-3"></i>079 2766 4785</p>
                    </div>
                    <!-- Grid column -->
                </div>
                <!-- Grid row -->
            </div>
        </section>
        <!-- Section: Links  -->

        <!-- Copyright -->
        <div class="text-center p-4" style="background-color: rgba(0, 0, 0, 0.05);">
            © 2021 Copyright:
            <a class="text-reset fw-bold" href="/">Laptop World</a>
        </div>
        <!-- Copyright -->
    </footer>
</body>
</html>